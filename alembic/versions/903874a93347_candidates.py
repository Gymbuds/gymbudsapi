...
def upgrade() -> None:
    ...
    op.create_table("match_candidates",sa.Column("id",sa.Integer(),primary_key=True),
                    ...
                    sa.Column("user_candidate_id",sa.Integer(),nullable=False),
                    sa.Column("status",sa.Enum('ACCEPTED', 'REJECTED', 'PENDING',name='candidate_status'), nullable=False),
                    sa.Column("score",sa.Integer(),nullable=False),
                    sa.ForeignKeyConstraint(["user_id"], ["users.id"], ondelete="CASCADE"),
                    sa.ForeignKeyConstraint(["user_candidate_id"], ["users.id"], ondelete="CASCADE"),
                    sa.UniqueConstraint("user_id", "user_candidate_id", name="candidate_user_pair")
                    )
